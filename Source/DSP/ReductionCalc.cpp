...
void ReductionCalc::prepare(const double newSampleRate)
{
...
void ReductionCalc::setAttackTime(const float attackTimeInSeconds)
{
    attackTime = attackTimeInSeconds;
    alphaAttack = 1.0f - timeToGain(attackTime);
}

void ReductionCalc::setReleaseTime(const float releaseTimeInSeconds)
{
    releaseTime = releaseTimeInSeconds;
    alphaRelease = 1.0f - timeToGain(releaseTime);
}

const float ReductionCalc::timeToGain(const float timeInSeconds)
{
    return std::exp(-1.0f / (static_cast<float> (sampleRate) * timeInSeconds));
}

void ReductionCalc::setKnee(const float kneeInDecibels)
{
    knee = kneeInDecibels;
    kneeHalf = knee / 2.0f;
}

void ReductionCalc::setThreshold(const float thresholdInDecibels)
{
    threshold = thresholdInDecibels;
}

void ReductionCalc::setMakeUpGain(const float makeUpGainInDecibels)
{
    makeUpGain = makeUpGainInDecibels;
}

void ReductionCalc::setRatio(const float ratio)
{
    slope = 1.0f / ratio - 1.0f;
}


inline const float ReductionCalc::applyCharacteristicToOverShoot(const float overShootInDecibels)
{
...
void ReductionCalc::computeGainInDecibelsFromSidechainSignal(const float* sideChainSignal, float* destination, const int numSamples)
{
    ...
    for (int i = 0; i < numSamples; ++i)
    {
        ...
        // calculate overshoot(amount in decibels by which the input signal exceeds the threshold level) and apply knee and ratio
...
void ReductionCalc::computeLinearGainFromSidechainSignal(const float* sideChainSignal, float* destination, const int numSamples)
{
...
void ReductionCalc::getCharacteristic(float* inputLevelsInDecibels, float* dest, const int numSamples)
{
    for (int i = 0; i < numSamples; ++i)
        dest[i] = getCharacteristicSample(inputLevelsInDecibels[i]);
}

float ReductionCalc::getCharacteristicSample(const float inputLevelInDecibels)
{
